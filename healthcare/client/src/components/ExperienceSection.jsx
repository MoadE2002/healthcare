...
const ExperienceSection = ({ userData, isOwnProfile, onSave , doctorId }) => {
  ...
  const handleAddExperience = async () => {
    if (newExperience.title && newExperience.company && newExperience.startDate) {
      try {
        const response = await axiosInstance.post(`/experience`, {
          doctor: doctorId,
          ...newExperience,
        });
        setExperiences([...experiences, response.data]);
        onSave([...experiences, response.data]);
  ...
  const handleDeleteExperience = async (index, experienceId) => {
    try {
  ...
  const handleSave = async () => {
    try {
      await Promise.all(experiences.map(exp => axiosInstance.put(`/experience/${exp._id}`, exp)));
      onSave(experiences);
      setIsEditing(false);
    } catch (error) {
    ...
    <div className="bg-white shadow rounded-lg p-6 mb-6">
      ...
      {experiences.map((exp, index) => (
        <div key={index} className="mb-4 flex justify-between items-start">
          ...
          {isEditing && (
            <button onClick={() => handleDeleteExperience(index, exp._id)} className="text-red-500">
              <X size={20} />
            </button>
          )}
        </div>
